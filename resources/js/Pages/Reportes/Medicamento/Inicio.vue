<script setup>
  ...
  const { openModal, Toast, Swal, formatoFecha, meses } = useHelper();
    ...
    const { registros, obtenerMedicamentos,errors } = usePrecioMedicamento();

    const titleHeader = ref({
      titulo: "Medicamentos - Precio",
      subTitulo: "Reportes",
      icon: "",
      vista: ""
    });
    ...
    const listarMedicamentos = async(page=1) => {
        ...
        await obtenerMedicamentos(dato.value, periodo.value.codigo)
    }
      ...
      <div class="container-fluid">
        <div class="card card-primary card-outline">
            <div class="card-header">
                <h6 class="card-title">
                    Precios de Medicamentos por Periodo
                </h6>
            </div>
            <div class="card-body">
                <div class="row mb-4">
                            ...
                            <table class="table table-bordered table-hover table-sm table-striped table-sm">
                                ...
                                <tbody>
                                    ...
                                    <tr v-else v-for="(medicamento,index) in registros.data" :key="medicamento.id">
                                        ...
                                        <td>{{ medicamento.med_CodMed }}</td>
                                        <td>{{ medicamento.med_Nombre }}</td>
                                        <td>{{ medicamento.med_FormaFarmaceutica }}</td>
                                        <td>{{ medicamento.med_Concen }}</td>
