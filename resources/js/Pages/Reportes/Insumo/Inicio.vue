<script setup>
  ...
  import usePrecioInsumo from '@/Composables/PrecioInsumo.js';
  import JsonExcel from 'vue-json-excel3';
  import html2canvas from 'html2canvas'
  import jspdf from 'jspdf'
  const { formatoFecha, meses } = useHelper();

    const { registros, obtenerInsumos,errors } = usePrecioInsumo();

    const titleHeader = ref({
      titulo: "Insumos - Precio",
      subTitulo: "Reportes",
      icon: "",
      vista: ""
    });
    ...
    const listarMedicamentos = async(page=1) => {
        ...
        await obtenerInsumos(dato.value, periodo.value.codigo)
    }
      ...
      <div class="container-fluid">
        <div class="card card-primary card-outline">
            <div class="card-header">
                <h6 class="card-title">
                    Precios de Insumos por Periodo
                </h6>
            </div>
            <div class="card-body">
                <div class="row mb-4">
                        ...
                        <div class="table-responsive">         
                            <table class="table table-bordered table-hover table-sm table-striped table-sm">
                                ...
                                <tbody>
                                    ...
                                    <tr v-else v-for="(medicamento,index) in registros.data" :key="medicamento.id">
                                        <td>{{ index + registros.from }}</td>
                                        <td>{{ medicamento.CPERIODO }}</td>
                                        <td>{{ medicamento.ins_CodIns }}</td>
                                        <td>{{ medicamento.ins_Nombre }}</td>
                                        <td>{{ medicamento.ins_FormaFarmaceutica }}</td>
                                        <td>{{ medicamento.ins_Concen }}</td>
                                        <td>{{ medicamento.PREADJ }}</td>
                                        <td>{{ medicamento.PREOPE }}</td>
                                        <td>{{ medicamento.CODIGO_SIGA }}</td>
                                        <td>{{ medicamento.TIPOCOMP }}</td>                                        
                                    </tr>
                                </tbody>
                            </table>
                        </div>
