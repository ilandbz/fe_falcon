<script setup>
  ...
  const {
        ...
        obtenerProcedimientos, obtenerProcedimiento, eliminarProcedimiento
    } = useProcedimiento();
    const {
    ...
    const listarProcedimientos = async(page=1) => {
        ...
        await obtenerProcedimientos(dato.value)
    }
    // PAGINACION
    const isActived = () => {
...
<template>
            ...
            <div class="card-body">
                        ...
                        <div class="table-responsive">         
                            <table class="table table-bordered table-hover table-sm table-striped">
                                <thead class="table-dark">
                                    <tr>
                                        <th colspan="6" class="text-center">Procedimientos</th>
                                    </tr>
                                    <tr>
                                        <th class="small p-1">#</th>
                                        <th class="small p-1">Codigo</th>
                                        <th class="small p-1">PRCD_V_CODPROCEDIMIENTO</th>
                                        <th class="small p-1">PRCD_V_CODPRO_MINSA</th>
                                        <th class="small p-1">Descripcion</th>
                                        <th class="small p-1">Acciones</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr v-if="procedimientos.total == 0">
                                        <td class="text-danger text-center" colspan="5">
                                            -- Datos No Registrados - Tabla Vacía --
                                        </td>
                                    </tr>
                                    <tr v-else v-for="(procedimiento,index) in procedimientos.data" :key="procedimiento.id">
                                        <td class="small p-0">{{ index + procedimientos.from }}</td>
                                        <td class="small p-0">{{ procedimiento.PRCD_ID_PROCEDIMIENTO }}</td>
                                        <td class="small p-0">{{ procedimiento.PRCD_V_CODPROCEDIMIENTO }}</td>
                                        <td class="small p-0">{{ procedimiento.PRCD_V_CODPRO_MINSA }}</td>
                                        <td class="small p-0">{{ procedimiento.PRCD_V_DESPROCEDIMIENTO }}</td>
                                        <td class="small p-0">
                                            <button class="btn btn-info btn-sm" title="Ver Detalle" @click.prevent="ver(procedimiento.id)">
                ...
                <div class="row">
                    <div class="col-md-5 mb-1">
                        Mostrando <b>{{procedimientos.from}}</b> a <b>{{ procedimientos.to }}</b> de <b>{{ procedimientos.total}}</b> Registros
                    </div>
    ...
    <detForm :objeto="registroProc"></detForm>
</template>